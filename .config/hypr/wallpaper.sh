...
main() {
    ...
    awww img "$selected_wallpaper" --outputs DP-2 --transition-type any --transition-fps 60 --transition-duration .5
    if [[ "$selected_wallpaper" == *.* ]]; then
      new="${selected_wallpaper%.*}1.${selected_wallpaper##*.}"
    else
      new="${selected_wallpaper}1"
    fi
    awww img "${new}" --outputs DP-3 --transition-type any --transition-fps 60 --transition-duration .5
